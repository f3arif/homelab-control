...
function Invoke-DesktopCommanderRecovery([string]$Action){
  if($Action -notin @('audit','apply')){throw 'unsupported Desktop Commander recovery action'}
  $runner=Join-Path $InstallRoot 'afz-openai-agent\Invoke-DesktopCommander-Recovery.ps1'
  if(-not(Test-Path -LiteralPath $runner -PathType Leaf)){throw "Desktop Commander recovery runner missing: $runner"}
  $raw=(& powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $runner -Action $Action 2>&1 | Out-String).Trim()
  $code=$LASTEXITCODE
  if([string]::IsNullOrWhiteSpace($raw)){throw 'Desktop Commander recovery returned empty output'}
  $parsed=$null
  try{$parsed=$raw|ConvertFrom-Json}catch{throw "Desktop Commander recovery returned invalid JSON: $raw"}
  if($code -ne 0){throw "Desktop Commander recovery failed exit=$code classification=$([string]$parsed.classification)"}
  return $parsed
}
...
$listener.Start()
Log "START version=1.10.3 port=$Port bind=$BindHost deploy=github-fast-signal interval=3s h3QwenBenchmark=typed queueOrphanRemediation=typed windowsWslMemoryAudit=typed-readonly jellyfinVisibilityRepair=typed hpEnvySurfsharkExitNode=typed-fixed-target prospectEngineProbe=enabled"
try{
  while($listener.IsListening){$ctx=$listener.GetContext();try{
    $ip=Get-RemoteIp $ctx;$path=$ctx.Request.Url.AbsolutePath.TrimEnd('/')

    if($path -eq '/api/desktop-commander-recovery' -and $ctx.Request.HttpMethod -eq 'POST'){
      if(-not(Test-DeployPeer $ip)){Send-Json $ctx 403 @{ok=$false;error='Desktop Commander recovery peer not authorized';client=$ip};continue}
      ...
      if($action -notin @('audit','apply')){Send-Json $ctx 400 @{ok=$false;error='unsupported Desktop Commander recovery action'};continue}
...
'@;Send-Html $ctx $html;continue}
 ...
 -or $sha -ne $current){Send-Json $ctx 409 @{ok=$false;error='Desktop Commander recovery source mismatch';current=$current;requested=$sha};continue}
      $r=Invoke-DesktopCommanderRecovery $action
      Log "Desktop Commander recovery action=$action classification=$([string]$r.classification) sha=$sha requested by $ip"
      Send-Json $ctx 200 $r
      continue
    }
