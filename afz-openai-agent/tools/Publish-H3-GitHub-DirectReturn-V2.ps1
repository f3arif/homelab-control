...
$publisherState=Join-Path $root 'return-publisher-v2.json'
...
function Get-PublicJson([string]$Url){
  $h=@{'User-Agent'='AFZ-H3-Return-V2';'Accept'='application/vnd.github+json';'Cache-Control'='no-cache'}
  Invoke-RestMethod -Uri $Url -Headers $h -TimeoutSec 30
}
...
function Find-ResultMarker{
  $candidates=@()
  if(Test-Path $root){$candidates+=@(Get-ChildItem -LiteralPath $root -File -Filter '*.stdout.log' -ErrorAction SilentlyContinue)}
...
function Resolve-ReturnState{
  ...
  if($local -and [string]$local.job_id -eq $job -and [string]$local.status -in @('running','completed','failed') -and $local.result){
    return [pscustomobject]@{job_id=$job;status=[string]$local.status;source_sha=$(if(([string]$local.source_sha)-match '^[0-9a-f]{40}$'){[string]$local.source_sha}else{$canon.MainSha});result=$local.result;error=$null;source='state.json'}
  }
  ...
  $err=$null;if($local -and $local.error){$err=[string]$local.error}
  if(-not $err){$err='No recoverable AFZ_RESULT_JSON payload is currently present. Return transport was repaired without rerunning Qwen.'}
  return [pscustomobject]@{job_id=$job;status='blocked';source_sha=$(if($local -and ([string]$local.source_sha)-match '^[0-9a-f]{40}$'){[string]$local.source_sha}else{$canon.MainSha});result=$null;error=$err;source='recovered-blocked'}
}
...
function Put-Json([string]$Path,$Object,[string]$Message){
  $json=$Object|ConvertTo-Json -Depth 40 -Compress;$payload=[ordered]@{message=$Message;content=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($json));branch=$ResultBranch}
  $sha=Get-RemoteSha $Path;if($sha){$payload.sha=$sha}
  $tmp=Join-Path $env:TEMP ('afz-return-put-'+[guid]::NewGuid().ToString('N')+'.json')
  try{[IO.File]::WriteAllText($tmp,($payload|ConvertTo-Json -Compress),$utf8);$r=Invoke-Gh @('api',"repos/$Repo/contents/$Path",'--method','PUT','--input',$tmp);if($r.ExitCode -ne 0){throw "GitHub Contents API PUT failed path=$Path exit=$($r.ExitCode): $($r.Stderr)"}}
  finally{Remove-Item $tmp -Force -ErrorAction SilentlyContinue}
}
function Try-Comment([string]$RepoName,[int]$Issue,[string]$Body){
  $tmp=Join-Path $env:TEMP ('afz-return-comment-'+[guid]::NewGuid().ToString('N')+'.txt')
  try{[IO.File]::WriteAllText($tmp,$Body,$utf8);$r=Invoke-Gh @('issue','comment',[string]$Issue,'--repo',$RepoName,'--body-file',$tmp);return ($r.ExitCode -eq 0)}finally{Remove-Item $tmp -Force -ErrorAction SilentlyContinue}
}

try{
  $resolved=Resolve-ReturnState
  $envelope=[ordered]@{schema=1;transport='h3-direct-return-v2';job_id=$resolved.job_id;status=$resolved.status;source_sha=$resolved.source_sha;worker='H3';host=$env:COMPUTERNAME;source=$resolved.source;result=$resolved.result;error=$resolved.error;recovered_at=(Get-Date -Format o)}
  Write-Json $returnEnvelope $envelope
  ...
  if($perm.ExitCode -ne 0){throw ('GitHub API authentication unavailable in interactive H3 publisher context: '+$(if($perm.Stderr){$perm.Stderr}else{$perm.Stdout}))}
  if($perm.Stdout.Trim().ToLowerInvariant() -ne 'true'){throw 'H3 GitHub identity lacks repository push permission.'}
  $message=if($resolved.result){"Recovered benchmark result from $($resolved.source) without rerunning Qwen."}else{"Return path repaired; current recoverable state is BLOCKED: $($resolved.error)"}
  $statusDoc=[ordered]@{schema=1;transport='h3-direct-github';kind='status';job_id=$resolved.job_id;source_sha=$resolved.source_sha;worker='H3';host=$env:COMPUTERNAME;status=$resolved.status;message=$message;published_at=(Get-Date -Format o)}
  Put-Json $statusPath $statusDoc "H3 direct benchmark status $($resolved.status) $($resolved.job_id)"
  $resultPublished=$false
  if($resolved.result){$resultDoc=[ordered]@{schema=1;transport='h3-direct-github';job_id=$resolved.job_id;source_sha=$resolved.source_sha;worker='H3';host=$env:COMPUTERNAME;result=$resolved.result;published_at=(Get-Date -Format o)};Put-Json $resultPath $resultDoc "H3 direct benchmark result $($resolved.job_id)";$resultPublished=$true}
  $comment="[H3-DIRECT RETURN V2] job=$($resolved.job_id); status=$($resolved.status); source=$($resolved.source); resultPublished=$resultPublished. Return-only recovery; Qwen was not rerun."
  $c1=Try-Comment $controlRepo $controlIssue $comment;$c2=Try-Comment $Repo $alertIssue $comment
  Write-Json $publisherState ([ordered]@{schema=2;ok=$true;job_id=$resolved.job_id;status=$resolved.status;source=$resolved.source;branch_published=$true;result_published=$resultPublished;comments_ok=($c1 -and $c2);updated_at=(Get-Date -Format o)})
  Write-Output ('AFZ_H3_RETURN_V2='+(@{ok=$true;job_id=$resolved.job_id;status=$resolved.status;source=$resolved.source;result_published=$resultPublished}|ConvertTo-Json -Compress))
}catch{
  $msg=$_.Exception.Message
  Write-Json $publisherState ([ordered]@{schema=2;ok=$false;error=$msg;updated_at=(Get-Date -Format o)})
  Write-Error $msg
  exit 1
}
