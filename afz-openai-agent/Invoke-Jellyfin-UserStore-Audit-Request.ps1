...
$helper=Join-Path $InstallRoot 'afz-openai-agent\tools\Jellyfin-HomeVideos-IndexCheck.ps1'
...
$diagFile=Join-Path $diagRoot 'JELLYFIN-HOMEVIDEOS-INDEX-CHECK-LATEST.txt'
...
if(-not(Test-Path -LiteralPath $helper -PathType Leaf)){throw "Jellyfin Home Videos index helper missing: $helper"}
$started=Get-Date
try{
