...
if($env:COMPUTERNAME -ne 'DESKTOP-10SKF0M'){throw "windows-main-only QA carrier; host=$env:COMPUTERNAME"}
...
$launcherUrl="https://raw.githubusercontent.com/f3arif/homelab-control/$ExpectedSha/afz-openai-agent/tools/Launch-H3-QwenRidge16K-QualityAudit.ps1"
$resultRoot='C:\Users\Faiz\AppData\Local\AFZ\H3QwenRidge16KQACarrier'
...
function Save-Result([bool]$Ok,[string]$Status,[string]$Message,$Extra=$null){
  $o=[ordered]@{ok=$Ok;status=$Status;message=$Message;jobId=$JobId;expectedSha=$ExpectedSha;host=$env:COMPUTERNAME;target='DESKTOP-H3R6CQN';transport='interactive-carrier+wake+strict-ssh+github-exact-sha';modelCalls=0;siteMutationAllowed=$false;updatedAt=(Get-Date -Format o)}
...
try{
  ...
  Save-Result $false 'running' 'QA carrier started; waking H3 if needed before strict SSH.'
  Send-H3Wake

  $online=$false
  foreach($i in 1..30){
    foreach($candidate in $h3Candidates){if(Test-Connection -ComputerName $candidate -Count 1 -Quiet -ErrorAction SilentlyContinue){$online=$true;break}}
    if($online){break}
...
[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12
`$uri='$launcherUrl'
`$tmp=Join-Path `$env:TEMP 'AFZ-H3-Ridge16K-QA-Launch-$ExpectedSha.ps1'
try{
  Invoke-WebRequest -Uri `$uri -OutFile `$tmp -UseBasicParsing -Headers @{'User-Agent'='AFZ-H3-Ridge16K-QA-Carrier';'Cache-Control'='no-cache';'Pragma'='no-cache'} -TimeoutSec 60
  `$tokens=`$null;`$errors=`$null
  [void][System.Management.Automation.Language.Parser]::ParseFile(`$tmp,[ref]`$tokens,[ref]`$errors)
  if(`$errors.Count -gt 0){throw ('H3 QA launcher parse failure: '+(`$errors.Message -join '; '))}
...
"@
  ...
  if($r.ExitCode -ne 0){throw "H3 QA carrier SSH failed exit=$($r.ExitCode) stderr=$($r.StdErr) stdout=$($r.StdOut)"}
  $line=@(($r.StdOut -split '\r?\n')|Where-Object {$_ -match '^\{.*\}$'}|Select-Object -Last 1)
  if(-not $line){throw "H3 QA carrier returned no JSON launch proof. stdout=$($r.StdOut) stderr=$($r.StdErr)"}
  $extra=$line|ConvertFrom-Json
  if(-not [bool]$extra.ok){throw 'H3 QA detached launcher returned ok=false'}
  Save-Result $true 'completed' 'H3 Ridge16K read-only QA launch proved under interactive credential carrier.' $extra
