...
Write-Output 'AFZ_JELLYFIN_API_DECODED_STATE_AUDIT_V2';Write-Output ('TIME='+(Get-Date -Format o));Write-Output 'READ_ONLY=true';Write-Output 'SECRET_EXPOSED=false'
$token=$null;$users=@();$authMode=$null;$matchedUser=$null
# Prefer a server API key. Read it locally in SQLite read-only mode and never print it.
...
'@
  ...
  try{if([IO.Path]::GetFileName($python) -match '^py(\.exe)?$'){& $python -3 $tmpPy $db $tmpJson}else{& $python $tmpPy $db $tmpJson};if($LASTEXITCODE -eq 0 -and (Test-Path $tmpJson)){$keys=@(Get-Content $tmpJson -Raw -Encoding UTF8|ConvertFrom-Json);Write-Output ('APIKEY_CANDIDATES='+$keys.Count);$tested=0;foreach($k in $keys){$tested++;try{$r=Invoke-RestMethod -Uri ($base+'/Users') -Headers @{'X-Emby-Token'=[string]$k} -TimeoutSec 6;if($null -ne $r){$token=[string]$k;$users=@($r);$authMode='api-key';break}}catch{}};Write-Output ('APIKEY_TESTED='+$tested)}}finally{Remove-Item -LiteralPath $tmpPy,$tmpJson -Force -ErrorAction SilentlyContinue}
 }
}
# If the configured API key is stale, recover the active interactive Jellyfin Web token from Chrome/Edge storage locally.
if(-not $token){
 $candidates=New-Object 'System.Collections.Generic.HashSet[string]' ([StringComparer]::Ordinal)
 $filesScanned=0;$indexedScanned=0
 ...
 foreach($t in $candidates){foreach($entry in $known.GetEnumerator()){try{$v=Invoke-RestMethod -Uri ($base+'/Users/'+$entry.Value+'/Views?IncludeExternalContent=true') -Headers @{'X-Emby-Token'=$t} -TimeoutSec 5;if($null -ne $v){$token=$t;$authMode='browser-session';$matchedUser=$entry.Key;try{$u=Invoke-RestMethod -Uri ($base+'/Users/'+$entry.Value) -Headers @{'X-Emby-Token'=$t} -TimeoutSec 5;$users=@($u)}catch{$users=@([pscustomobject]@{Name=$entry.Key;Id=$entry.Value;Configuration=$null;Policy=$null})};break}}catch{}};if($token){break}}
}
