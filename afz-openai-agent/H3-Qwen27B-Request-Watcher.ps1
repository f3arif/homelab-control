...
try{
  ...
  Log "START interval=${IntervalSeconds}s role=bootstrap-only target=H3-direct-github"
  while($true){
    try{
      if(Test-Path $requestFile){
        ...
        elseif($handled -and [string]$handled.status -eq 'bootstrapping' -and [int]$handled.bootstrapPid -gt 0 -and (Get-Process -Id ([int]$handled.bootstrapPid) -ErrorAction SilentlyContinue)){
          # One-time bootstrap still active.
        }
        else{
          ...
          Log "BOOTSTRAP_START job=$jobId pid=$($p.Id) sha=$sha"
        }
      }
    }catch{
      $msg=$_.Exception.Message;Log "ERROR $msg";Save-WatchState '' 'error' $msg
    }
