...
try{
  ...
  if($LASTEXITCODE -ne 0){throw "Hermes clean installer failed exit=$LASTEXITCODE"}
}finally{Remove-Item -LiteralPath $installer -Force -ErrorAction SilentlyContinue}

$launcher=Get-Launcher
if(-not $launcher){throw 'Neither hermes.exe nor hermes.cmd was staged after clean install.'}
$git=(Get-Command git.exe -ErrorAction SilentlyContinue|Select-Object -First 1)
if(-not $git){throw 'Git executable missing after install.'}
...
Save-State ([ordered]@{schema=1;ok=$true;status='ready';classification='HERMES_ASUS_FALLBACK_READY_LOCAL_OLLAMA_64K';host=$env:COMPUTERNAME;user=$env:USERNAME;hermesHome=$hermesHome;installDir=$installDir;preservedIncompleteInstall=$backupDir;installerTemplateBackup=$templateBackup;launcher=$launcher;version=$version;commit=$pin;commitVerified=$true;provider='custom';baseUrl='http://127.0.0.1:11434/v1';model=$model;configuredContextLength=$contextLength;modelNativeContext=$modelContext;gatewayStarted=$false;generationTestStarted=$false;ollamaExposureChanged=$false;finishedAt=(Get-Date -Format o)})
