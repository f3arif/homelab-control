function Invoke-AFZSiteR5HungSshAbortSafe {
    ...
    $watchStatus=[string]$watch.status
    $exactR5Failed=($watchStatus -eq 'failed' -and ([string]$watch.message) -eq 'Carrier task ended without a matching deployment result file.')
    if(($watchStatus -ne 'deploying' -and -not $exactR5Failed) -or [string]$watch.jobId -ne $jobId -or ([string]$watch.expectedSiteSha).ToLowerInvariant() -ne $siteSha){
        throw 'Website deploy watcher is neither exact R5 deploying nor the exact known R5 carrier-ended failure state; abort refused.'
    }
    $summary.Add("Watcher state                 : $watchStatus")
