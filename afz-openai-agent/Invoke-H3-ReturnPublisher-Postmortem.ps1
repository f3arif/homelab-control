...
$marker=Join-Path $root 'postmortem-v1.json'
...
if(-not $g -or [int]$g.generation -ne 5 -or [string]$g.status -ne 'completed' -or -not [bool]$g.ok){
  $na=[ordered]@{schema=1;ok=$false;status='not-applicable';reason='generation-5-not-completed';syncedSha=$SyncedSha;readOnly=$true;updatedAt=(Get-Date -Format o)}
  Save $na
  exit 0
}
if(-not(Test-Path -LiteralPath $key -PathType Leaf)){Save ([ordered]@{schema=1;ok=$false;status='failed';stage='local-key';error="SYSTEM H3 key missing: $key";syncedSha=$SyncedSha;readOnly=$true;updatedAt=(Get-Date -Format o)});exit 0}
if(-not(Test-Path -LiteralPath $known -PathType Leaf)){Save ([ordered]@{schema=1;ok=$false;status='failed';stage='local-known-hosts';error="H3 known-hosts missing: $known";syncedSha=$SyncedSha;readOnly=$true;updatedAt=(Get-Date -Format o)});exit 0}
if(-not(Test-Path -LiteralPath $ssh -PathType Leaf)){Save ([ordered]@{schema=1;ok=$false;status='failed';stage='local-ssh';error="ssh.exe missing: $ssh";syncedSha=$SyncedSha;readOnly=$true;updatedAt=(Get-Date -Format o)});exit 0}
...
function Run-GhRead([string]$Gh,[string[]]$GhArgs){
  ...
  $o=Join-Path $env:TEMP ('afz-h3-post-gh-out-'+[guid]::NewGuid().ToString('n')+'.txt')
  $e=Join-Path $env:TEMP ('afz-h3-post-gh-err-'+[guid]::NewGuid().ToString('n')+'.txt')
  try{
...
[ordered]@{
  ...
  publisherHasGhArgsFix=$(if($publisherText){[bool]($publisherText -match 'function Invoke-Gh\(\[string\[\]\]\$GhArgs\)')}else{$false});
  publisherStillHasArgsBug=$(if($publisherText){[bool]($publisherText -match 'function Invoke-Gh\(\[string\[\]\]\$Args\)')}else{$false});
...
$out=Join-Path $env:TEMP ('afz-h3-postmortem-out-'+[guid]::NewGuid().ToString('n')+'.txt')
$err=Join-Path $env:TEMP ('afz-h3-postmortem-err-'+[guid]::NewGuid().ToString('n')+'.txt')
try{
  ...
  if(-not $p.WaitForExit(60000)){try{$p.Kill()}catch{};try{$p.WaitForExit()}catch{};$r=[ordered]@{schema=1;ok=$false;status='failed';stage='ssh';error='postmortem timeout';syncedSha=$SyncedSha;readOnly=$true;updatedAt=(Get-Date -Format o)};Save $r;exit 0}
  ...
  $r=[ordered]@{schema=1;ok=$ok;status=$(if($ok){'captured'}else{'failed'});syncedSha=$SyncedSha;target='DESKTOP-H3R6CQN';transport='system-ssh-encoded-command';readOnly=$true;sshExit=$sshExit;remote=$remoteResult;stdout=$(if($remoteResult){$null}else{$stdout});stderr=$stderr.Trim();updatedAt=(Get-Date -Format o)}
  Save $r
  exit 0
