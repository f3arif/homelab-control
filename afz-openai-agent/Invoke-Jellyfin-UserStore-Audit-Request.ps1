...
$helper=Join-Path $InstallRoot 'afz-openai-agent\tools\Jellyfin-Preference-Presentation-Audit.ps1'
...
$diagFile=Join-Path $diagRoot 'JELLYFIN-PREFERENCE-PRESENTATION-AUDIT-LATEST.txt'
...
if(-not(Test-Path -LiteralPath $helper -PathType Leaf)){throw "Jellyfin preference presentation helper missing: $helper"}
$started=Get-Date
try{
