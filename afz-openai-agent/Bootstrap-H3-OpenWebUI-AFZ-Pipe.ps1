...
$runnerUrl="https://raw.githubusercontent.com/f3arif/homelab-control/$ExpectedSha/afz-openai-agent/openwebui/Run-H3-OpenWebUI-AFZ-Pipe.ps1"
...
function Save-State([string]$Status,[string]$Message,$Extra=$null){
  $o=[ordered]@{
    ...
    transport='windows-main-ssh+github-exact-sha-runner'
...
try{
  ...
  Save-State 'running' 'Starting bounded H3 exact-SHA OpenWebUI runner over strict-host-key SSH.'

  # Keep the SSH command line deliberately small. The large OpenWebUI installation
  # logic lives in the exact-SHA H3 runner and helper files in GitHub.
  $remote=@"
`$ErrorActionPreference='Stop'
if(`$env:COMPUTERNAME -ne 'DESKTOP-H3R6CQN'){throw "Wrong host: `$env:COMPUTERNAME"}
[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12
`$uri='$runnerUrl'
`$tmp=Join-Path `$env:TEMP 'AFZ-H3-OpenWebUI-Runner-$ExpectedSha.ps1'
try{
  Invoke-WebRequest -Uri `$uri -OutFile `$tmp -UseBasicParsing -Headers @{'User-Agent'='AFZ-H3-OpenWebUI-Runner';'Cache-Control'='no-cache'} -TimeoutSec 60
  `$tokens=`$null;`$errors=`$null
  [void][System.Management.Automation.Language.Parser]::ParseFile(`$tmp,[ref]`$tokens,[ref]`$errors)
  if(`$errors.Count -gt 0){throw ('H3 OpenWebUI runner parse failure: '+(`$errors.Message -join '; '))}
...
"@
  ...
  if($encoded.Length -gt 6000){throw "Encoded H3 launcher unexpectedly large: $($encoded.Length) characters"}
  ...
  $sshResult=Invoke-NativeCapture -FilePath $ssh -Arguments $args -TimeoutSeconds 360
  if($sshResult.ExitCode -ne 0){
    throw "H3 OpenWebUI bootstrap SSH failed exit=$($sshResult.ExitCode) stderr=$($sshResult.StdErr) stdout=$($sshResult.StdOut)"
  }

  $line=@(($sshResult.StdOut -split '\r?\n')|Where-Object {$_ -match '^\{.*\}$'}|Select-Object -Last 1)
  if(-not $line){throw "H3 OpenWebUI bootstrap returned no JSON result. stdout=$($sshResult.StdOut) stderr=$($sshResult.StdErr)"}
  $extra=$line|ConvertFrom-Json
  if(-not [bool]$extra.ok){throw 'H3 OpenWebUI runner returned ok=false'}

  Save-State 'completed' 'AFZ Typed Agent Pipe installed/enabled in OpenWebUI and smoke-tested through bounded exact-SHA runner transport.' $extra
