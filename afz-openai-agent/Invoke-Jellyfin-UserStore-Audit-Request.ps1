...
$helper=Join-Path $InstallRoot 'afz-openai-agent\tools\Jellyfin-HomeVideos-Wedding-DbReconcile.ps1'
...
$diagFile=Join-Path $diagRoot 'JELLYFIN-HOME-WEDDING-DB-RECONCILE-LATEST.txt'
...
if(-not(Test-Path -LiteralPath $helper -PathType Leaf)){throw "Jellyfin Home/Wedding DB reconciliation helper missing: $helper"}
$started=Get-Date
try{
