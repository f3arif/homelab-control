...
$helper=Join-Path $InstallRoot 'afz-openai-agent\tools\Jellyfin-DeviceToken-ApiAudit.ps1'
...
$diagFile=Join-Path $diagRoot 'JELLYFIN-DEVICE-TOKEN-API-AUDIT-LATEST.txt'
...
if(-not(Test-Path -LiteralPath $helper -PathType Leaf)){throw "Jellyfin device-token audit helper missing: $helper"}
$started=Get-Date
try{
