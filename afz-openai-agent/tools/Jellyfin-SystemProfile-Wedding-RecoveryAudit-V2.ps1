...
Write-Output 'AFZ_JELLYFIN_SYSTEMPROFILE_WEDDING_RECOVERY_AUDIT_V2'
...
try:
 ...
 def C(n): return '"'+low[n]+'"' if n in low else 'NULL'
 def g(r,n): return r.get(low.get(n,'')) if low.get(n,'') else None
 def norm(v): return ('' if v is None else str(v)).replace('-','').lower()
 rows=[dict(r) for r in c.execute('select '+','.join(C(x) for x in ('id','name','type','path','parentid','topparentid','data'))+' from BaseItems')]
 hits=[]; physical=[]
 for r in rows:
  name=str(g(r,'name') or ''); typ=str(g(r,'type') or ''); path=str(g(r,'path') or '')
  if 'wedding' in name.lower() or ('wedding' in path.lower() and ('CollectionFolder' in typ or 'Folder' in typ)): hits.append(r)
 print('WEDDING_DB_HIT_COUNT=%d'%len(hits))
 for r in hits:
  data=str(g(r,'data') or '')
  print('WEDDING_DB_HIT|id=%s|name=%s|type=%s|path=%s|parent=%s|top=%s|data=%s'%(g(r,'id'),g(r,'name'),g(r,'type'),g(r,'path'),g(r,'parentid'),g(r,'topparentid'),data.replace('\r',' ').replace('\n',' ')))
 ...
 for pid in sorted(set(physical)):
  ms=[r for r in rows if norm(g(r,'id'))==pid.replace('-','').lower()]
  ...
  foreach($d in $dirs){
    ...
    foreach($f in @(Get-ChildItem -LiteralPath $d.FullName -File -Force -ErrorAction SilentlyContinue)){
      if($f.Extension -ieq '.mblink'){
        ...
        $targetExists=$false
        if($v -and $v -ne 'READ_FAILED'){$targetExists=Test-Path -LiteralPath $v}
        Write-Output ('WEDDING_MBLINK|file='+$f.Name+'|value='+$v+'|targetExists='+$targetExists)
