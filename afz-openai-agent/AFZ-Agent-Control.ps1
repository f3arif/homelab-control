...
$listener=New-Object Net.HttpListener;$listener.Prefixes.Add("http://127.0.0.1:$Port/");if($BindHost -and $BindHost -ne '127.0.0.1'){$listener.Prefixes.Add("http://$BindHost`:$Port/")};$listener.Start();Log "START version=1.1.0 port=$Port bind=$BindHost"
try{
...
'@;Send-Html $ctx $html;continue}
    if($path -eq '/health' -and $ctx.Request.HttpMethod -eq 'GET'){Send-Json $ctx 200 [ordered]@{ok=$true;service='AFZ-Agent-Control';version='1.1.0';commit=(Get-Commit);transport='github-zip-no-git';updateTask=(Get-ScheduledTask -TaskName 'AFZ OpenAI Agent Updater' -ErrorAction SilentlyContinue).State;time=(Get-Date -Format o)};continue}
