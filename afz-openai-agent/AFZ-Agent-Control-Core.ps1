...
'@;Send-Html $ctx $html;continue}
 ...
 -or $sha -ne $current){Send-Json $ctx 409 @{ok=$false;error='H3 Ollama repair source mismatch';current=$current;requested=$sha};continue}
      $r=Invoke-H3OllamaRepair0332
      Log "H3 Ollama 0.33.2 repair action=apply classification=$([string]$r.classification) sha=$sha requested by $ip"
      Send-Json $ctx 200 $r
      continue
    }
...
'@;Send-Html $ctx $html;continue}
 ...
 -or $sha -ne $current){Send-Json $ctx 409 @{ok=$false;error='queue remediation source mismatch';current=$current;requested=$sha};continue}
      $r=Invoke-QueueOrphanRemediation $action $taskId;Log "queue orphan remediation action=$action task=$taskId sha=$sha requested by $ip";Send-Json $ctx 200 $r;continue
    }
