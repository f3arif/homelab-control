...
$helper=Join-Path $InstallRoot 'afz-openai-agent\tools\Jellyfin-VirtualFolders-Inventory.ps1'
...
$diagFile=Join-Path $diagRoot 'JELLYFIN-VIRTUALFOLDERS-INVENTORY-LATEST.txt'
...
if(-not(Test-Path -LiteralPath $helper -PathType Leaf)){throw "Jellyfin virtual folders inventory helper missing: $helper"}
$started=Get-Date
try{
