...
function Invoke-H3([string]$RemoteScript){
  ...
  try{
    ...
    if(-not $p.WaitForExit(90000)){
      try{$p.Kill()}catch{}
      throw 'H3 post-return bootstrap timed out after 90 seconds.'
    }
    return [ordered]@{
      exit=[int]$p.ExitCode
...
if(-not(Test-Path -LiteralPath `$statePath -PathType Leaf)){throw "Authoritative 35B state missing: `$statePath"}
`$state=Get-Content -LiteralPath `$statePath -Raw -Encoding UTF8|ConvertFrom-Json -ErrorAction Stop
if([string]`$state.job_id -ne `$jobId){throw 'Authoritative 35B state job id mismatch.'}
if([string]`$state.status -eq 'completed'){
  [pscustomobject]@{ok=`$true;classification='QWEN35B_POSTRETURN_ALREADY_COMPLETED';jobId=`$jobId;modelCallIssuedByRecovery=`$false;authoritativeState=`$state}|ConvertTo-Json -Depth 30 -Compress
  exit 0
}
if([string]`$state.phase -eq 'recovery_failed'){
  [pscustomobject]@{ok=`$false;classification='QWEN35B_POSTRETURN_RECOVERY_FAILED_ON_H3';jobId=`$jobId;modelCallIssuedByRecovery=`$false;authoritativeState=`$state}|ConvertTo-Json -Depth 30 -Compress
  exit 20
}
`$attempted=`$false
if(`$state.PSObject.Properties.Name -contains 'model_call_attempted'){`$attempted=[bool]`$state.model_call_attempted}
`$curlExit=-1
if(`$state.PSObject.Properties.Name -contains 'curl_exit'){`$curlExit=[int]`$state.curl_exit}
if(-not `$attempted -or [string]`$state.phase -ne 'ollama_post_returned' -or `$curlExit -ne 0){
  [pscustomobject]@{ok=`$false;classification='QWEN35B_POSTRETURN_GUARD_NOT_SATISFIED';jobId=`$jobId;modelCallIssuedByRecovery=`$false;authoritativeState=`$state}|ConvertTo-Json -Depth 30 -Compress
  exit 21
}
...
[pscustomobject]@{
  ok=`$true
  classification='QWEN35B_POSTRETURN_RECOVERY_STARTED_FROM_SAVED_RESPONSE'
  jobId=`$jobId
...
"@
  ...
  $proof=$null
  if($jsonLine){$proof=([string]$jsonLine).Trim()|ConvertFrom-Json -ErrorAction Stop}
  if([int]$r.exit -ne 0){
    if($proof){
      Save-State ([ordered]@{schema=1;status='failed';classification=[string]$proof.classification;jobId=$jobId;recoverySha=$recoverySha;modelCallIssuedByRecovery=$false;h3Proof=$proof;sshExit=[int]$r.exit;sshStderr=[string]$r.stderr;time=(Get-Date -Format o)})
      exit 20
    }
    throw "H3 post-return bootstrap failed exit=$($r.exit) stdout=$($r.stdout) stderr=$($r.stderr)"
  }
  if(-not $proof){throw "H3 post-return bootstrap returned no JSON. stdout=$($r.stdout) stderr=$($r.stderr)"}
  if(-not [bool]$proof.ok){throw ('H3 post-return bootstrap returned ok=false: '+[string]$proof.classification)}
  $status=$(if([string]$proof.classification -eq 'QWEN35B_POSTRETURN_ALREADY_COMPLETED'){'completed'}else{'started'})
  Save-State ([ordered]@{schema=1;status=$status;classification=[string]$proof.classification;jobId=$jobId;recoverySha=$recoverySha;modelCallIssuedByRecovery=$false;stateSource='H3_LOCAL_STRICT_SSH';h3Proof=$proof;time=(Get-Date -Format o)})
  exit 0
}catch{
  Save-State ([ordered]@{schema=1;status='failed';classification='QWEN35B_POSTRETURN_SYSTEM_CARRIER_FAILED';jobId=$jobId;recoverySha=$recoverySha;modelCallIssuedByRecovery=$false;stateSource='H3_LOCAL_STRICT_SSH';error=$_.Exception.Message;detail=($_|Out-String).Trim();time=(Get-Date -Format o)})
  exit 20
}
