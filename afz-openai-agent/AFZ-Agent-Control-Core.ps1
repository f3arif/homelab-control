...
){throw 'Commander alternate-account request id invalid'}
...
'@;Send-Html $ctx $html;continue}
 ...
 -or $sha -ne $current){Send-Json $ctx 409 @{ok=$false;error='Commander pairing source mismatch';current=$current;requested=$sha};continue}
