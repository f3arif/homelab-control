...
function Invoke-CommanderAlternateAccountPair {
  ...
  if(-not(Test-Path -LiteralPath $runner -PathType Leaf)){throw "Commander alternate-account launcher missing: $runner"}
  if(-not(Test-Path -LiteralPath $request -PathType Leaf)){throw "Commander alternate-account request missing: $request"}
  $contract=Get-Content -LiteralPath $request -Raw -Encoding UTF8|ConvertFrom-Json
  if([int]$contract.schema -ne 1 -or [string]$contract.status -ne 'ACTIVE' -or [string]$contract.action -ne 'pair-alternate-commander-account'){throw 'Commander alternate-account request is not active'}
  $id=([string]$contract.id).Trim()
  if($id -notmatch '^[A-Za-z0-9][A-Za-z0-9._-]{2,120}$'){throw 'Commander alternate-account request id invalid'}
  if(-not [bool]$contract.windows_main -or -not [bool]$contract.h3){throw 'Commander alternate-account request targets are not both enabled'}
  & powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $runner -InstallRoot $InstallRoot -RequestPath $request | Out-Null
  $code=$LASTEXITCODE
  if($code -ne 0){throw "Commander alternate-account launcher failed exit=$code"}
  $statePath=Join-Path 'C:\ProgramData\AFZ\OpenAIAgent\jobs\commander-alternate-account-pair' ($id+'.json')
  $state=$null
...
try{
  while($listener.IsListening){$ctx=$listener.GetContext();try{
    ...
    if($path -eq '/api/commander-alternate-account-pair' -and $ctx.Request.HttpMethod -eq 'POST'){
      $allowed=((Get-AllowedClients) -contains $ip)
      if($ip -notin @('100.71.26.69','100.106.186.118') -or -not $allowed){Send-Json $ctx 403 @{ok=$false;error='Commander pairing Hermes peer not authorized';client=$ip};continue}
      $req=Read-Json $ctx;$action=([string]$req.action).Trim().ToLowerInvariant()
      $repo=[string]$req.repository;$ref=[string]$req.ref;$sha=([string]$req.sha).Trim().ToLowerInvariant();$current=([string](Get-Commit)).Trim().ToLowerInvariant()
      if($action -ne 'launch-active-request'){Send-Json $ctx 400 @{ok=$false;error='unsupported Commander pairing action'};continue}
      if($repo -ne 'f3arif/homelab-control' -or $ref -ne 'refs/heads/main' -or $sha -notmatch '^[0-9a-f]{40}$' -or $sha -ne $current){Send-Json $ctx 409 @{ok=$false;error='Commander pairing source mismatch';current=$current;requested=$sha};continue}
      $r=Invoke-CommanderAlternateAccountPair
      Log "Commander alternate-account pairing action=launch-active-request requestId=$([string]$r.requestId) sha=$sha requested by $ip"
      Send-Json $ctx 202 $r
      continue
    }
