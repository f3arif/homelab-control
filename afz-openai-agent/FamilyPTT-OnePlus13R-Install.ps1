...
function Adb([string]$adb,[string[]]$args,[switch]$AllowFailure){
...
function Adb-Timed([string]$adb,[string[]]$args,[int]$seconds=120){
  ...
  try{
    ...
    if(-not $p.WaitForExit($seconds*1000)){try{Stop-Process -Id $p.Id -Force}catch{};throw "adb install timed out after ${seconds}s"}
    $stdout=$(if(Test-Path $out){Get-Content -LiteralPath $out -Raw}else{''});$stderr=$(if(Test-Path $err){Get-Content -LiteralPath $err -Raw}else{''})
...
function Get-Prop([string]$adb,[string]$serial,[string]$prop){$r=Adb $adb @('-s',$serial,'shell','getprop',$prop) -AllowFailure;return (($r.output -join '').Trim())}
...
$result=[ordered]@{schema=1;jobId=$job;status='running';ok=$false;classification='ONEPLUS13R_INSTALL_RUNNING';startedAt=(Get-Date -Format o);finishedAt=$null;apkSha256=$null;device=$null;installResult=$null;pid=$null;recordAudioGranted=$false;error=$null}
Save $result
try{
  if(-not(Test-Path -LiteralPath $apkPath -PathType Leaf)){throw 'validated Phase 1 APK is not staged on windows-main'}
  ...
  $serials=@($devices.output|ForEach-Object{if(([string]$_) -match '^([^\s]+)\s+device$'){$Matches[1]}}|Where-Object{$_})
  $matches=@()
  ...
  $install=Adb-Timed $adb @('-s',$target.serial,'install','--no-streaming','-r',$apkPath) 120
  ...
  $null=Adb $adb @('-s',$target.serial,'shell','pm','grant',$package,'android.permission.RECORD_AUDIO') -AllowFailure
  $perm=Adb $adb @('-s',$target.serial,'shell','dumpsys','package',$package) -AllowFailure
  $result.recordAudioGranted=(($perm.output -join "`n") -match 'android\.permission\.RECORD_AUDIO:\s+granted=true')
  $null=Adb $adb @('-s',$target.serial,'shell','am','force-stop',$package)
  $null=Adb $adb @('-s',$target.serial,'shell','am','start','-W','-n',$component)
  Start-Sleep -Seconds 7
  $pid=Adb $adb @('-s',$target.serial,'shell','pidof',$package) -AllowFailure;$result.pid=(($pid.output -join '').Trim())
  ...
  $result.ok=$true;$result.status='completed';$result.classification='ONEPLUS13R_FAMILYPTT_INSTALLED_READY';$result.finishedAt=(Get-Date -Format o);Save $result;exit 0
}catch{
