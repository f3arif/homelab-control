...
foreach($p in @($agent,$wrapper,$control,$update,$pushWatcher,$stremioRequestWatcher,$stremioRunner,$stremioHelper,$allowFile)){if(-not(Test-Path $p)){throw "Required AFZ agent file missing: $p"}}
...
$stremioWatcherTask='AFZ Stremio Organize Request Watcher';$stremioWatcherAction=New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "-NoProfile -ExecutionPolicy Bypass -File `"$stremioRequestWatcher`" -InstallRoot `"$InstallRoot`" -IntervalSeconds 5";Register-ScheduledTask -TaskName $stremioWatcherTask -Action $stremioWatcherAction -Trigger (New-ScheduledTaskTrigger -AtStartup) -Settings $serviceSettings -Principal $principal -Force|Out-Null
...
Start-ScheduledTask -TaskName $agentTask;Start-ScheduledTask -TaskName $controlTask;Start-ScheduledTask -TaskName $pushWatcherTask;Start-ScheduledTask -TaskName $stremioWatcherTask
