...
# Safety assertions: one early initializer, no late initializer after Save-State,
# and the model single-flight markers must remain untouched.
if s.count(init) != 1:
    raise SystemExit(f'expected exactly one early GitHub initializer, found {s.count(init)}')
for required in (
