...
function Invoke-AFZRadioHilalHermesMaintenanceIfRequested {
  ...
  if($sha -notmatch '^[0-9a-f]{40}$'){throw 'RadioHilal maintenance could not resolve an exact GitHub main SHA.'}
  ...
  if($jobId -notmatch '^radiohilal-hermes-cron-rh1-[A-Za-z0-9._-]+$'){throw 'RadioHilal maintenance job id is not allowlisted.'}
  ...
  try{
    ...
    $success=($exitCode -eq 0 -and $allText -match '(?m)^SMOKE=PASS\s*$' -and $allText -match '(?m)^TEST=PASS\s*$' -and $allText -match '(?m)^FINAL_STATE=RESUMED\s*$' -and $allText -match '(?m)^PINNED_PROVIDER=ollama\s*$' -and $allText -match '(?m)^PINNED_MODEL=qwen3\.8-ridge:27b-64k-rh1\s*$')
