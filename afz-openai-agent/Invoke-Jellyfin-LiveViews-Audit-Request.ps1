...
$helper=Join-Path $InstallRoot 'afz-openai-agent\tools\Jellyfin-BrowserToken-LiveViews-Audit.ps1'
...
$diagFile=Join-Path $diagRoot 'JELLYFIN-LIVE-VIEWS-AUDIT-LATEST.txt'
...
if([int]$req.schema -ne 1 -or [string]$req.project -ne 'jellyfin-liveviews-audit' -or [string]$req.action -ne 'audit-browser-token-live-views-readonly'){throw 'Invalid Jellyfin liveviews audit request'}
