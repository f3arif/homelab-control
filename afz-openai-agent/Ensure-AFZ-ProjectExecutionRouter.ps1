...
try{
  foreach($p in @($watcher,$executor,$router)){if(-not(Test-Path -LiteralPath $p -PathType Leaf)){throw "Required router source missing: $p"}}
  foreach($p in @($watcher,$executor)){
  ...
  # so it adds no OneDrive/network polling load.
  ...
  $watchSettings=New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -RestartCount 20 -RestartInterval (New-TimeSpan -Seconds 10) -ExecutionTimeLimit ([TimeSpan]::Zero) -MultipleInstances IgnoreNew
  ...
  if($existing){Set-ScheduledTask -TaskName $watcherTask -Action $watchAction -Trigger $watchTrigger -Settings $watchSettings | Out-Null;$watchActionState='updated'}
  else{Register-ScheduledTask -TaskName $watcherTask -Action $watchAction -Trigger $watchTrigger -Settings $watchSettings -Principal $watchPrincipal -Force | Out-Null;$watchActionState='registered'}
  ...
  $userSettings=New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -ExecutionTimeLimit (New-TimeSpan -Hours 3) -MultipleInstances IgnoreNew
  $existingUser=Get-ScheduledTask -TaskName $userTask -ErrorAction SilentlyContinue
  if($existingUser){Set-ScheduledTask -TaskName $userTask -Action $userAction -Settings $userSettings | Out-Null;$userActionState='updated'}
