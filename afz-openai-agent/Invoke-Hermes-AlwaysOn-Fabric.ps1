...
if([string]$req.primary.provider -ne 'openai-codex' -or [string]$req.primary.model -ne 'gpt-5.6-luna' -or [string]$req.primary.provider_source -ne 'hermes-managed-oauth-auth-json'){throw 'HP Envy Codex OAuth policy mismatch.'}
if([string]$req.accelerator.preferred_model -ne 'qwen3.6:35b-a3b-hermes64k' -or [int]$req.accelerator.context_length -ne 65536){throw 'H3 fallback model policy mismatch.'}
if([string]$req.accelerator.fallback_provider -ne 'custom:h3-qwen35b' -or [string]$req.accelerator.fallback_base_url -ne 'http://100.106.186.118:11438/v1'){throw 'H3 fallback route mismatch.'}
if([string]$req.accelerator.adapter_health_url -ne 'http://100.106.186.118:11438/health' -or [string]$req.accelerator.fallback_transport -ne 'tailscale-serve-loopback-openai-adapter'){throw 'H3 adapter transport mismatch.'}
...
auth="$home_dir/.hermes/auth.json"
config_present=false
config_provider=""
config_model=""
config_context=0
config_base_url_present=false
...
if [ "$hermes_present" = true ] && [ -r "$config" ]; then
  config_present=true
  model_json="$("$hermes_launcher" config get model --json 2>/dev/null || true)"
  read -r config_provider config_model config_context config_base_url_present <<EOF
$(printf '%s' "$model_json" | python3 -c 'import json,sys; d=json.load(sys.stdin); m=d.get("model",d); print(m.get("provider",""),m.get("default",m.get("model","")),m.get("context_length",0),"true" if bool(m.get("base_url")) else "false")' 2>/dev/null || printf '  0 false')
EOF
  fallback_text="$("$hermes_launcher" fallback list 2>/dev/null || true)"
  if printf '%s\n' "$fallback_text" | grep -Fq 'qwen3.6:35b-a3b-hermes64k' &&
     printf '%s\n' "$fallback_text" | grep -Fq 'custom:h3-qwen35b' &&
     printf '%s\n' "$fallback_text" | grep -Fq 'http://100.106.186.118:11438/v1'; then
    fallback_present=true
    fallback_provider='custom:h3-qwen35b'
    fallback_model='qwen3.6:35b-a3b-hermes64k'
    fallback_base_url='http://100.106.186.118:11438/v1'
  fi
fi
...
def usable(v):
    ...
    if isinstance(v,dict): return any(usable(x) for x in v.values())
    if isinstance(v,list): return any(usable(x) for x in v)
    return False
def walk(v,path=()):
...
adapter_model_present=false
if command -v curl >/dev/null 2>&1; then
  health="$(curl -fsS --connect-timeout 4 --max-time 8 'http://100.106.186.118:11438/health' 2>/dev/null || true)"
  printf '%s' "$health" | grep -q '"ok":true' && adapter_health=true
  models="$(curl -fsS --connect-timeout 4 --max-time 8 'http://100.106.186.118:11438/v1/models' 2>/dev/null || true)"
  printf '%s' "$models" | grep -Fq 'qwen3.6:35b-a3b-hermes64k' && adapter_model_present=true
fi

gateway_running=false
if [ "$hermes_present" = true ]; then
  gateway_text="$("$hermes_launcher" gateway status 2>&1 || true)"
  printf '%s\n' "$gateway_text" | grep -Eqi 'gateway.*running|running.*gateway' && gateway_running=true
fi
...
export AFZ_HERMES="$hermes_present" AFZ_HERMES_LAUNCHER="$hermes_launcher" AFZ_HERMES_VERSION="$hermes_version" AFZ_CONFIG="$config_present"
export AFZ_CONFIG_PROVIDER="$config_provider" AFZ_CONFIG_MODEL="$config_model" AFZ_CONFIG_CONTEXT="$config_context" AFZ_CONFIG_BASE="$config_base_url_present"
export AFZ_AUTH="$auth_verified" AFZ_FB="$fallback_present" AFZ_FB_PROVIDER="$fallback_provider" AFZ_FB_MODEL="$fallback_model" AFZ_FB_URL="$fallback_base_url"
export AFZ_ADAPTER="$adapter_health" AFZ_ADAPTER_MODEL="$adapter_model_present" AFZ_GATEWAY="$gateway_running"
...
print(json.dumps({
 ...
 "hermesPresent":b("AFZ_HERMES"),"hermesLauncher":os.environ.get("AFZ_HERMES_LAUNCHER",""),"hermesVersion":os.environ.get("AFZ_HERMES_VERSION",""),"configPresent":b("AFZ_CONFIG"),
 "configProvider":os.environ.get("AFZ_CONFIG_PROVIDER",""),"configModel":os.environ.get("AFZ_CONFIG_MODEL",""),"configContextLength":i("AFZ_CONFIG_CONTEXT"),"configBaseUrlPresent":b("AFZ_CONFIG_BASE"),
 "openAiCodexAuthVerified":b("AFZ_AUTH"),"fallbackPresent":b("AFZ_FB"),"fallbackProvider":os.environ.get("AFZ_FB_PROVIDER",""),"fallbackModel":os.environ.get("AFZ_FB_MODEL",""),"fallbackBaseUrl":os.environ.get("AFZ_FB_URL",""),
 "adapterHealth":b("AFZ_ADAPTER"),"adapterModelPresent":b("AFZ_ADAPTER_MODEL"),"gatewayRunning":b("AFZ_GATEWAY"),
...
$hpFallbackReady=$false
if([bool]$hp.sshOk){
  $hpIdentityOk=([string]$hp.data.user -eq 'coolyo' -and [string]$hp.data.tailscaleIp -eq '100.71.26.69')
  $hpPrereqs=([bool]$hp.data.git -and [bool]$hp.data.curl -and [bool]$hp.data.python3 -and [bool]$hp.data.systemd -and [bool]$hp.data.hermesPresent)
  $hpProviderReady=(
    [string]$hp.data.configProvider -eq 'openai-codex' -and
    [string]$hp.data.configModel -eq 'gpt-5.6-luna' -and
    [int]$hp.data.configContextLength -eq 65536 -and
    -not [bool]$hp.data.configBaseUrlPresent -and
    [bool]$hp.data.openAiCodexAuthVerified
  )
  $hpFallbackReady=(
    [bool]$hp.data.fallbackPresent -and
    [string]$hp.data.fallbackProvider -eq 'custom:h3-qwen35b' -and
    [string]$hp.data.fallbackModel -eq 'qwen3.6:35b-a3b-hermes64k' -and
    [string]$hp.data.fallbackBaseUrl -eq 'http://100.106.186.118:11438/v1' -and
    [bool]$hp.data.adapterHealth -and
    [bool]$hp.data.adapterModelPresent
  )
}
$asusModelReady=([bool]$local.identityOk -and [bool]$local.ollamaReachable -and @($local.hermesCandidates).Count -gt 0)
$ready=($hpIdentityOk -and $hpPrereqs -and $hpProviderReady -and $hpFallbackReady -and $asusModelReady)
