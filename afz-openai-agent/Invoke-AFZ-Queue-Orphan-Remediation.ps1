...
function Get-ResultStatus([System.IO.FileInfo]$File){
  ...
  try{
    $line=Get-Content -LiteralPath $File.FullName -TotalCount 40 -ErrorAction Stop |
...
function Get-SearchRoots{
  ...
  $oneDrive=Join-Path $env:USERPROFILE 'OneDrive - AFZ Engineering Inc'
  $shared=Join-Path $oneDrive 'AFZ Shared'
  foreach($r in @(
    (Join-Path $shared 'AFZ Workers'),
    (Join-Path $shared 'AFZ Queue'),
    (Join-Path $shared 'AFZ Results')
  )){if(Test-Path -LiteralPath $r -PathType Container){[void]$roots.Add($r)}}
  if(Test-Path -LiteralPath $shared -PathType Container){
    Get-ChildItem -LiteralPath $shared -Directory -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -match 'Queue|Worker|Result|Processing|Hold|Staging|Failed|Completed|Quarantine' } |
      ForEach-Object { if(-not $roots.Contains($_.FullName)){[void]$roots.Add($_.FullName)} }
  }
  @($roots)
}
...
function Find-TaskArtifacts([string]$Needle){
  ...
  foreach($root in @(Get-SearchRoots)){
    try{
      Get-ChildItem -LiteralPath $root -Recurse -File -Filter "$Needle*" -ErrorAction SilentlyContinue |
        ForEach-Object {
          if(-not $seen.ContainsKey($_.FullName)){
            $seen[$_.FullName]=$true
            [void]$items.Add([pscustomobject]@{
              file=$_
              classification=(Get-ArtifactClass $_)
              resultStatus=(Get-ResultStatus $_)
            })
          }
        }
    }catch{}
...
if($Action -eq 'audit'){
  [ordered]@{
    ...
    janitor=$janitor;liveExecutors=$live;terminalEvidence=$terminalSummary;movableArtifacts=$movableSummary
