...
param(
  ...
  [string]$RequestId='h3-afz-blog-checkout-v2',
...
function Invoke-H3([string]$RemoteScript){
  ...
  $inFile=Join-Path $env:TEMP ('afz-h3-blog-v2-'+[guid]::NewGuid().ToString('n')+'.ps1')
  $outFile=Join-Path $env:TEMP ('afz-h3-blog-v2-'+[guid]::NewGuid().ToString('n')+'.out')
  $errFile=Join-Path $env:TEMP ('afz-h3-blog-v2-'+[guid]::NewGuid().ToString('n')+'.err')
  ...
  try{
    ...
    if(-not $p.WaitForExit(90000)){try{$p.Kill()}catch{};throw 'H3 AFZ Blog checkout v2 timed out after 90 seconds.'}
...
try{
  ...
  elseif([string]$identity.Name -ieq ($expectedWindowsHost+'\Faiz')){$key=$userKey;$relayIdentity='Faiz'}
...
function Test-GitHubAccess {
  ...
  if(-not $gitCmd){return [ordered]@{accessible=$false;timedOut=$false;exitCode=$null}}
  $gitPath=$(if($gitCmd.Path){[string]$gitCmd.Path}else{[string]$gitCmd.Source})
  $outFile=Join-Path $env:TEMP ('afz-blog-git-access-'+[guid]::NewGuid().ToString('n')+'.out')
  $errFile=Join-Path $env:TEMP ('afz-blog-git-access-'+[guid]::NewGuid().ToString('n')+'.err')
  $oldGcm=$env:GCM_INTERACTIVE;$oldPrompt=$env:GIT_TERMINAL_PROMPT
  try{
    $env:GCM_INTERACTIVE='Never';$env:GIT_TERMINAL_PROMPT='0'
    $p=Start-Process -FilePath $gitPath -ArgumentList @('ls-remote','--exit-code',$repo,'HEAD') -RedirectStandardOutput $outFile -RedirectStandardError $errFile -PassThru -WindowStyle Hidden
    if(-not $p.WaitForExit(20000)){
      try{$p.Kill()}catch{}
      return [ordered]@{accessible=$false;timedOut=$true;exitCode=$null}
    }
    return [ordered]@{accessible=([int]$p.ExitCode -eq 0);timedOut=$false;exitCode=[int]$p.ExitCode}
  }catch{
    return [ordered]@{accessible=$false;timedOut=$false;exitCode=$null}
  }finally{
...
function Emit([bool]$ok,[string]$classification,$extra,[int]$code){
  $o=[ordered]@{schema=2;ok=$ok;classification=$classification;host=$env:COMPUTERNAME;action=$action;repository=$repo;workspace=$workspace;expectedSha=$expectedSha;remoteMutationStarted=$mutationStarted;credentialsEmitted=$false;time=(Get-Date -Format o)}
...
if(-not(Get-Command git.exe -ErrorAction SilentlyContinue)){Emit $false 'H3_AFZ_BLOG_GIT_MISSING' @{} 31}
$before=Snapshot
$access=Test-GitHubAccess
if($action -eq 'audit'){Emit $true 'H3_AFZ_BLOG_AUDIT_COMPLETE' @{githubAccess=$access;before=$before} 0}

if($before.workspaceExists){
  if(-not $before.gitRepo){Emit $false 'H3_AFZ_BLOG_EXISTING_PATH_NOT_GIT' @{githubAccess=$access;before=$before} 32}
  $originNorm=([string]$before.origin).TrimEnd('/').ToLowerInvariant()
  if($originNorm -notin @('https://github.com/f3arif/afz-blog.git','git@github.com:f3arif/afz-blog.git')){Emit $false 'H3_AFZ_BLOG_REMOTE_MISMATCH' @{githubAccess=$access;before=$before} 33}
  if([int]$before.dirtyCount -ne 0){Emit $false 'H3_AFZ_BLOG_EXISTING_DIRTY' @{githubAccess=$access;before=$before} 34}
  if(-not [bool]$access.accessible){Emit $false 'H3_AFZ_BLOG_GITHUB_AUTH_MISSING_OR_BLOCKED' @{githubAccess=$access;before=$before} 35}
  $mutationStarted=$true
  $oldGcm=$env:GCM_INTERACTIVE;$oldPrompt=$env:GIT_TERMINAL_PROMPT
  try{$env:GCM_INTERACTIVE='Never';$env:GIT_TERMINAL_PROMPT='0';Native @('-C',$workspace,'fetch','--prune','origin','main')|Out-Null}
  ...
  if($remoteSha -ne $expectedSha){Emit $false 'H3_AFZ_BLOG_REMOTE_SHA_UNEXPECTED' @{githubAccess=$access;before=$before;originMain=$remoteSha} 36}
  ...
  $mutationStarted=$true
  $oldGcm=$env:GCM_INTERACTIVE;$oldPrompt=$env:GIT_TERMINAL_PROMPT
  try{$env:GCM_INTERACTIVE='Never';$env:GIT_TERMINAL_PROMPT='0';Native @('clone','--branch','main','--single-branch',$repo,$workspace)|Out-Null}
  finally{$env:GCM_INTERACTIVE=$oldGcm;$env:GIT_TERMINAL_PROMPT=$oldPrompt}
}
$after=Snapshot
if(-not $after.gitRepo -or [string]$after.head -ne $expectedSha -or [string]$after.branch -ne 'main' -or [int]$after.dirtyCount -ne 0){Emit $false 'H3_AFZ_BLOG_VERIFY_FAILED' @{githubAccess=$access;before=$before;after=$after} 37}
if((Test-Path -LiteralPath (Join-Path $workspace '.env') -PathType Leaf) -or (Test-Path -LiteralPath (Join-Path $workspace '.env.local') -PathType Leaf)){Emit $false 'H3_AFZ_BLOG_SECRET_FILE_PRESENT' @{githubAccess=$access;before=$before;after=$after} 38}
Emit $true 'H3_AFZ_BLOG_CHECKOUT_READY' @{githubAccess=$access;before=$before;after=$after} 0
'@
  ...
  if(-not $jsonLine){throw "H3 checkout v2 returned no JSON. exit=$($r.exit) stderr=$($r.stderr)"}
  ...
  $safe=[ordered]@{schema=2;ok=[bool]$payload.ok;classification=[string]$payload.classification;requestId=$RequestId;relayHost=$env:COMPUTERNAME;relayIdentity=$relayIdentity;target='h3';host=[string]$payload.host;action=$Action;repository=[string]$payload.repository;workspace=[string]$payload.workspace;expectedSha=$ExpectedBlogSha;githubAccess=$payload.githubAccess;remoteMutationStarted=[bool]$payload.remoteMutationStarted;credentialsEmitted=$false;before=$payload.before;after=$payload.after;sshExit=[int]$r.exit;time=(Get-Date -Format o)}
  Save-State $safe
  if(-not $safe.ok){exit 42}
  exit 0
}catch{
  Save-State ([ordered]@{schema=2;ok=$false;classification='H3_AFZ_BLOG_RELAY_FAILED';requestId=$RequestId;relayHost=$env:COMPUTERNAME;relayIdentity=$relayIdentity;target='h3';action=$Action;expectedSha=$ExpectedBlogSha;credentialsEmitted=$false;error=$_.Exception.Message;time=(Get-Date -Format o)})
  exit 43
}
